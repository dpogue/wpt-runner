...
class SubHandler {
  ...
  handleRequest(request, response) {
    try {
      const path = filesystemPath(this.basePath, request, this.urlBase);
      const f = fs.readFileSync(path, { encoding: "utf8" });

      const { pathname, search } = new URL(request.url, `http://${request.headers.host}`);

      const props = search.substring(1).split('&').map(_ => _.split('=')).map(([k,v]) => ({[k]: v})).reduce((a, b) => Object.assign(a, b), {});

      const content = f.replace(/\{\{GET\[(.+)\]\}\}/mg, (match, prop) => props[prop]);
